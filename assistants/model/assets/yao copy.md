...
模型标识是 Yao 框架中用于唯一标识模型实例的字符串。它由模型文件的路径和文件名组成，遵循以下规则：

1. 模型文件位于 `models` 目录下
2. 文件路径中的斜杠 `/` 会被替换为点号 `.`
3. 文件扩展名 `.yao` 或 `.json` 会被移除

例如：
- `models/user.yao` 的模型标识为 `user`
- `models/my/name.yao` 的模型标识为 `my.name`
- `models/auto/car.yao` 的模型标识为 `auto.car`
